...
class Player(pg.sprite.Sprite):
    ...
    def get_keys(self):
        self.mouse = vec(pg.mouse.get_pos()[0], pg.mouse.get_pos()[1])
        self.rot_speed = 0
        self.vel = vec(0, 0)
    ...
    def update(self):
        self.get_keys()
        self.rot = (self.mouse - self.pos_rel).angle_to(vec(1, 0))
...
class Mob(pg.sprite.Sprite):
    ...
    def draw_health(self):
        if self.health > (MOB_HEALTH * 6 / 10):
            col = GREEN
        elif self.health > (MOB_HEALTH * 3 / 10):
            col = YELLOW
        else:
            col = RED
